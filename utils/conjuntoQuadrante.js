...
export default function createConjuntoQuadrante(indice) {
  ...
  conjuntoQuadrante.adicionarConjunto = (conjunto) => {
    ...
    for (let iConjunto = 0; iConjunto < 3; iConjunto++) {
      ...
      conjuntoQuadrante.conjuntosPosicoes[iUltimoElemento].push(
        conjuntoPosicoes
      );
    }
  };
                  ...
                  for (let iCol = 0; iCol < 3; iCol++) {
                    if (iLin !== iLinConjunto && iColConjunto !== iCol) {
                      const conjuntoPosicoes =
                        conjuntoQuadrante.conjuntosPosicoes[iLin][iCol];

                      if (conjuntoPosicoes.possuiValor(possivelValor)) {
                        qtdeVezesValorEncontrado++;
                      } else {
                        const estaDentreValoresPossiveis =
                          conjuntoPosicoes.possivelValor(possivelValor);
                        let conjuntosAdjacentesNaoPossuem = true;

                        for (let iLinAdj = 0; iLinAdj < 3; iLinAdj++) {
                          if (iLin !== iLinAdj) {
                            const conjuntoAdjacente =
                              conjuntoQuadrante.conjuntosPosicoes[iLinAdj][
                                iCol
                              ];

                            if (
                              conjuntoAdjacente.possivelValor(possivelValor)
                            ) {
